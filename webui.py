...
if os.path.exists("runtime"):
    ...
    espeak_library = os.path.join(
        os.path.dirname(__file__), "espeak NG", "libespeak-ng.dll"
    )
...
VALID_AUDIO_EXT = [".mp3", ".wav", ".flac", ".aac", ".ogg", ".m4a", ".opus"]
...
def load_all_models(model_path):
    ...
    model_state.model, model_state.model_params = load_models_webui(
        sigma_value, model_state.device
    )
    model_state.phonemizer = load_phonemizer()

    model_state.sampler = create_sampler(model_state.model)
    model_state.text_cleaner = TextCleaner()
    model_state.to_mel = torchaudio.transforms.MelSpectrogram(
        n_mels=80, n_fft=2048, win_length=1200, hop_length=300
    )

    model_state.params_whole = load_pretrained_model(
        model_state.model, model_path=model_path
    )
    return model_state
...
def unload_all_models():
    for k in list(model_state):
        if k != "device" and model_state[k] is not None:
            model_state[k] = None
            print(f"Unloaded {k}")
...
def generate_audio(
    ...
    for k, path in reference_dicts.items():
        mean, std = -4, 4
        print(f"model:{model_state.model}")
        ref_s = compute_style(
            path, model_state.model, model_state.to_mel, mean, std, model_state.device
        )
        ...
        for t in texts:
            audios.append(
                inference(
                    ...
                    model_state.model,
                    model_state.sampler,
                    model_state.text_cleaner,
                    model_state.to_mel,
                    model_state.device,
                    model_state.model_params,
                    global_phonemizer=model_state.phonemizer,
                    alpha=alpha,
                    beta=beta,
...
def update_reference_audio(voice):
    return gr.Dropdown(
        choices=get_reference_audio_list(voice),
        value=get_reference_audio_list(voice)[0],
    )
...
def update_button_proxy():
    ...
    return (
        gr.Dropdown(choices=voice_list_with_defaults),
        gr.Dropdown(choices=datasets_list),
        gr.Dropdown(choices=train_list),
        gr.Dropdown(choices=train_list),
    )
...
def save_yaml_config(config, voice_name):
    os.makedirs(
        os.path.join(TRAINING_DIR, voice_name), exist_ok=True
    )  # Create the output directory if it doesn't exist
    output_file_path = os.path.join(
        TRAINING_DIR, voice_name, f"{voice_name}_config.yml"
    )
    ...
    try:
        ...
        if os.path.exists(training_dir):
            ...
            if processed_dataset_list:
                ...
                return gr.Dropdown(
                    choices=processed_dataset_list, value="", interactive=True
                )
    except Exception as e:
...
def load_whisper_model(language=None, model_name=None, progress=None):
    ...
    try:
        whisper_model = whisperx.load_model(
            model_name, device, download_root="whisper_models", compute_type="float16"
        )
    except Exception as e:  # for older GPUs
        print(f"Debugging info: {e}")
        whisper_model = whisperx.load_model(
            model_name, device, download_root="whisper_models", compute_type="int8"
        )
...
def transcribe_other_language_proxy(
    voice,
    language,
    chunk_size,
    ...
    large_file_num_processes = int(
        num_processes / 2
    )  # Used for instances where larger files are being processed, as to not run out of RAM
    ...
    from modules.tortoise_dataset_tools.audio_conversion_tools.split_long_file import (
        get_duration,
        process_folder,
    )
    ...
    items = [
        ...
        if os.path.splitext(item)[1].lower() in VALID_AUDIO_EXT
    ]
    ...
    file_durations = [
        get_duration(os.path.join(chosen_directory, item))
        for item in items
    ...
    for item in items_to_move:
        ...
        if os.path.exists(move_item_path):
            try:
                ...
                raise gr.Error(
                    f'Close out of any windows using where "{item} is located!'
                )
...
def main():
    ...
    with gr.Blocks() as demo:
        with gr.Tabs():
                ...
                with gr.Column():
                    with gr.Row():
                        GENERATE_SETTINGS["text"] = gr.Textbox(
                            label="Input Text", value=initial_settings["text"]
                        )
                    with gr.Row():
                        with gr.Column():
                            GENERATE_SETTINGS["voice"] = gr.Dropdown(
                                choices=voice_list_with_defaults,
                                label="Voice",
                                type="value",
                            ...
                            GENERATE_SETTINGS["reference_audio_file"] = gr.Dropdown(
                                choices=ref_audio_file_choices,
                                label="Reference Audio",
                                type="value",
                                value=initial_settings["reference_audio_file"],
                            )
                        with gr.Column():
            ...
            with gr.TabItem("Training"):
                with gr.Tabs():
                    with gr.TabItem("Prepare Dataset"):
                        with gr.Column():
                            ...
                            DATASET_SETTINGS["voice"] = gr.Dropdown(
                                ...
                                type="value",
                                value=(
                                    datasets_list[0] if len(datasets_list) > 0 else ""
                                ),
                            )
                            DATASET_SETTINGS["continue_directory"] = gr.Dropdown(
                                choices=[],
                                label="Continuation Directory",
                                value="",
                                interactive=True,
                            )
                            ...
                            with gr.Row():
                                ...
                                DATASET_SETTINGS["num_processes"] = gr.Textbox(
                                    label="Processes to Use",
                                    value=int(max(1, multiprocessing.cpu_count()) - 2),
                                )
                            ...
                            with gr.Row(visible=False) as advanced_settings_row:
                                DATASET_SETTINGS["srt_multiprocessing"] = gr.Checkbox(
                                    label="Disable if dataset files are < 20s",
                                    value=True,
                                )
                            ...
                            phonemize_button = gr.Button(value="Run Phonemization")

                            archive_button = gr.Button(value="Archive Existing")
                        with gr.Column():
                            transcribe2_output = gr.Textbox(label="Progress Console")

                    def toggle_advanced_settings(show):
                        if show == "Show Advanced Settings":
                            return gr.update(value="Hide Advanced Settings"), gr.update(
                                visible=True
                            )
                        else:
                            return gr.update(value="Show Advanced Settings"), gr.update(
                                visible=False
                            )
                    ...
                    archive_button.click(
                        archive_dataset, inputs=[DATASET_SETTINGS["voice"]]
                    )
                                ...
                                with gr.Column():
                                    if train_list:
                                        ...
                                        train_data_path = (
                                            ...
                                            if os.path.exists(
                                                os.path.join(
                                                    training_dir, "train_phoneme.txt"
                                                )
                                            )
                                            else ""
                                        )
                                        ...
                                        audio_data_path = (
                                            os.path.join(
                                                TRAINING_DIR, train_list[0], "audio"
                                            )
                                            if os.path.exists(
                                                os.path.join(
                                                    TRAINING_DIR, train_list[0], "audio"
                                    ...
                                    refresh_available_config_button = gr.Button(
                                    ...
                                    pretrained_model = gr.Textbox(
                                        ...
                                        value=r"models\pretrain_base_1\epochs_2nd_00020.pth",
                                    )
                                    load_only_params = gr.Checkbox(
                                        value=True, label="Load Only Params"
                                    )

                                    diff_epoch = gr.Number(
                                        label="Diffusion Epoch", value=0
                                    )
                                    joint_epoch = gr.Number(
                                        label="Joint Epoch", value=0
                                    )

                                with gr.Column():
                                    F0_path = gr.Textbox(
                                        label="F0 Path", value=r"Utils\JDC\bst.t7"
                                    )
                                    ASR_config = gr.Textbox(
                                        label="ASR Config",
                                        value=r"Utils\ASR\config.yml",
                                    )
                                    ASR_path = gr.Textbox(
                                        label="ASR Path",
                                        value=r"Utils\ASR\epoch_00080.pth",
                                    )
                                    PLBERT_dir = gr.Textbox(
                                        label="PLBERT Directory", value=r"Utils\PLBERT"
                                    )
                                    train_data = gr.Textbox(
                                        label="Train Data",
                                        placeholder="Enter train data path",
                                        value=train_data_path,
                                    )
                                    val_data = gr.Textbox(
                                        label="Validation Data",
                                        placeholder="Enter validation data path",
                                        value=val_data_path,
                                    )
                                    root_path = gr.Textbox(
                                        label="Root Path",
                                        placeholder="Enter root path",
                                        value=audio_data_path,
                                    )
                    ...
                    def get_training_config(voice):
                        config_path = os.path.join(
                            "training", voice, f"{voice}_config.yml"
                        )
                        return config_path

                    def start_training_proxy(
                        voice, progress=gr.Progress(track_tqdm=True)
                    ):
                        from styletts2.train_finetune_accelerate import (
                            main as run_train,
                        )
                    ...
                    def launch_tensorboard_proxy():
                        ...
                        if is_port_in_use(port):
                            gr.Warning(
                                f"Port {port} is already in use. Skipping TensorBoard launch."
                            )
                        else:
                    ...
                    with gr.TabItem("Run Training"):
                        with gr.Row():
                            with gr.Column():
                                ...
                                refresh_available_config_button_2 = gr.Button(
                                    value="Refresh Available"
                                )
                            with gr.Column():
                        ...
                        with gr.Row():
                            launch_tensorboard_button = gr.Button(
                                value="Launch Tensorboard"
                            )
            ...
            refresh_available_config_button.click(
                update_button_proxy,
                outputs=[
                    GENERATE_SETTINGS["voice"],
                    DATASET_SETTINGS["voice"],
            ...
            voice_name.change(
                update_data_proxy,
                inputs=voice_name,
                outputs=[train_data, val_data, root_path],
            )
            ...
            with gr.TabItem("Settings"):
                ...
                GENERATE_SETTINGS["voice_model"] = gr.Dropdown(
                    ...
                    label="Voice Models",
                    type="value",
                    value=initial_settings["voice_model"],
                )
                ...
                refresh_models_available_button.click(
                    fn=update_models, outputs=GENERATE_SETTINGS["voice_model"]
                )
                unload_all_models_button = gr.Button(value="Unload all loaded models")

                unload_all_models_button.click(fn=unload_all_models)

                GENERATE_SETTINGS["voice_model"].change(
                    fn=update_voice_model, inputs=[GENERATE_SETTINGS["voice_model"]]
                )
